...
def launch_gui_worker(all_alive, current_output, process_names):
    p = GUIWorker(all_alive, current_output, process_names)
    p.daemon = True
    p.start()
    return p
...
class GUIWorker(Process):
    ...
    def run(self):
        WindowApp(self.alive_flag, tk.Tk(), self.current_output, self.process_names)
