...
from brainrender_gui_mod.app_mod import *
...
def launch(*args, output=None, **kwargs):
    ...
    ex = AppMod(*args, screenshot_kwargs=screenshot_kwargs, **kwargs)
...
def clilaunch(atlas=None, axes=False, output=None):
    ...
    app = QApplication(sys.argv)
    app.setApplicationName("Brainrender GUIs")
    ex = AppMod(atlas=atlas, axes=axes, screenshot_kwargs=screenshot_kwargs)
    app.aboutToQuit.connect(ex.onClose)
    ex.show()
    sys.exit(app.exec_())
